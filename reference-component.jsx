...
function PlayerCard({ player, players, theme, format, onUpdate, onRemove, isMinimized, onToggleMinimize }) {
  ...
  if (isMinimized) {
    return (
      <div onClick={onToggleMinimize} style={{
    ...
    <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 3 }}>
            {Object.entries(MANA_COLORS).map(([key, mc]) => (
              <div key={key} onClick={() => onUpdate({ color: key })} style={{ width: 14, height: 14, borderRadius: "50%", background: mc.color, cursor: "pointer", border: player.color === key ? `2px solid ${theme.text}` : "2px solid transparent", transition: "all 0.2s" }} />
            ))}
          </div>
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          <button onClick={onToggleMinimize} style={{ background: "none", border: "none", color: theme.muted, cursor: "pointer", fontSize: 16, padding: 2 }}>−</button>
          {players.length > 2 && (
            <button onClick={onRemove} style={{ background: "none", border: "none", color: "#7F1D1D", cursor: "pointer", fontSize: 14, padding: 2 }}>✕</button>
          )}
        </div>
      </div>
      ...
      <div style={{ padding: "8px 16px 12px", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 6, marginTop: 10 }}>
          {lifeButtons.map((amt) => (
            <button key={amt} onClick={() => onUpdate({ life: player.life + amt })} style={{
      ...
      <div style={{ display: "flex", borderTop: `1px solid ${theme.border}` }}>
        <button onClick={() => setShowCounters(!showCounters)} style={{
        ...
        {format.id === "commander" && (
          <button onClick={() => setShowCommander(!showCommander)} style={{
      ...
      {showCounters && (
        <div style={{ padding: 12, borderTop: `1px solid ${theme.border}`, background: "rgba(0,0,0,0.2)" }}>
          {[
            { label: "Poison", key: "poison", icon: <SkullIcon size={14} />, color: "#84CC16", max: 10 },
            ...
            <div key={counter.key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 0" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <button onClick={() => onUpdate({ [counter.key]: Math.max(0, player[counter.key] - 1) })} style={{ width: 28, height: 28, borderRadius: 6, background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.2)", color: "#F87171", fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>−</button>
                ...
                <button onClick={() => onUpdate({ [counter.key]: player[counter.key] + 1 })} style={{ width: 28, height: 28, borderRadius: 6, background: "rgba(74,222,128,0.1)", border: "1px solid rgba(74,222,128,0.2)", color: "#4ADE80", fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
              </div>
            </div>
          ))}
        </div>
          ...
          {players.filter((p) => p.id !== player.id).map((opp) => {
            ...
            return (
              <div key={opp.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "4px 0", opacity: lethal ? 0.5 : 1 }}>
                <span style={{ color: lethal ? "#EF4444" : theme.text, fontSize: 13 }}>{opp.name} {lethal && "💀"}</span>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <button onClick={() => { const newDmg = { ...player.commanderDamage, [opp.id]: Math.max(0, dmg - 1) }; onUpdate({ commanderDamage: newDmg }); }} style={{ width: 24, height: 24, borderRadius: 4, background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.2)", color: "#F87171", fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>−</button>
                  ...
                  <button onClick={() => { const newDmg = { ...player.commanderDamage, [opp.id]: dmg + 1 }; onUpdate({ commanderDamage: newDmg, life: player.life - 1 }); }} style={{ width: 24, height: 24, borderRadius: 4, background: "rgba(74,222,128,0.1)", border: "1px solid rgba(74,222,128,0.2)", color: "#4ADE80", fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
                </div>
              </div>
            );
          })}
      ...
      {isDead && (
        <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", fontSize: 48, opacity: 0.3, pointerEvents: "none" }}>💀</div>
      )}
    </div>
  );
}
...
export default function MTGTracker() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: theme.bg, fontFamily: "'Cinzel', serif", color: theme.text, maxWidth: 600, margin: "0 auto", position: "relative" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, padding: "16px 16px 100px" }}>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap", justifyContent: "center" }}>
          <button onClick={() => setShowSettings(!showSettings)} style={{ padding: "6px 14px", borderRadius: 8, background: showSettings ? theme.accent : "transparent", border: `1px solid ${theme.border}`, color: showSettings ? theme.bg : theme.muted, fontSize: 11, cursor: "pointer", letterSpacing: "0.08em", fontFamily: "'Cinzel', serif" }}>⚙ Settings</button>
          <button onClick={() => setShowTools(!showTools)} style={{ padding: "6px 14px", borderRadius: 8, background: showTools ? theme.accent : "transparent", border: `1px solid ${theme.border}`, color: showTools ? theme.bg : theme.muted, fontSize: 11, cursor: "pointer", letterSpacing: "0.08em", fontFamily: "'Cinzel', serif" }}>🎲 Tools</button>
          <button onClick={() => setShowHistory(!showHistory)} style={{ padding: "6px 14px", borderRadius: 8, background: showHistory ? theme.accent : "transparent", border: `1px solid ${theme.border}`, color: showHistory ? theme.bg : theme.muted, fontSize: 11, cursor: "pointer", letterSpacing: "0.08em", fontFamily: "'Cinzel', serif" }}><HistoryIcon size={12} /> Log</button>
        </div>
        ...
        {showSettings && (
          <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 12, padding: 16, marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {THEMES.map((t) => (
                <button key={t.id} onClick={() => setTheme(t)} style={{ padding: "6px 12px", borderRadius: 8, background: theme.id === t.id ? t.accent : "transparent", border: `1px solid ${theme.id === t.id ? t.accent : t.border}`, color: theme.id === t.id ? t.bg : t.accent, fontSize: 11, cursor: "pointer", fontFamily: "'Cinzel', serif" }}>{t.name}</button>
              ))}
            </div>
          </div>
        )}
              ...
              <div style={{ textAlign: "center" }}>
                ...
                <div style={{ display: "flex", gap: 4, justifyContent: "center", marginBottom: 8 }}>
                  {[4, 6, 8, 10, 12, 20].map((d) => (
                    <button key={d} onClick={() => setDiceType(d)} style={{ width: 30, height: 26, borderRadius: 4, background: diceType === d ? theme.accent : "transparent", border: `1px solid ${diceType === d ? theme.accent : theme.border}`, color: diceType === d ? theme.bg : theme.muted, fontSize: 10, cursor: "pointer", fontFamily: "'Cinzel', serif" }}>d{d}</button>
                  ))}
                </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 16, borderTop: `1px solid ${theme.border}`, paddingTop: 16 }}>
              <div style={{ textAlign: "center" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
                  <button onClick={() => setStormCount(Math.max(0, stormCount - 1))} style={{ width: 28, height: 28, borderRadius: 6, background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.2)", color: "#F87171", fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>−</button>
                  <span style={{ fontSize: 28, fontWeight: 700, color: theme.accent, minWidth: 30 }}>{stormCount}</span>
                  <button onClick={() => setStormCount(stormCount + 1)} style={{ width: 28, height: 28, borderRadius: 6, background: "rgba(74,222,128,0.1)", border: "1px solid rgba(74,222,128,0.2)", color: "#4ADE80", fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
                </div>
              </div>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 10, color: theme.muted, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 6 }}>Turn</div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
                  <button onClick={() => setTurnCount(Math.max(1, turnCount - 1))} style={{ width: 28, height: 28, borderRadius: 6, background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.2)", color: "#F87171", fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>−</button>
                  <span style={{ fontSize: 28, fontWeight: 700, color: theme.accent, minWidth: 30 }}>{turnCount}</span>
                  <button onClick={() => { setTurnCount(turnCount + 1); setStormCount(0); logAction(`Turn ${turnCount + 1}`); }} style={{ width: 28, height: 28, borderRadius: 6, background: "rgba(74,222,128,0.1)", border: "1px solid rgba(74,222,128,0.2)", color: "#4ADE80", fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
                </div>
              </div>
            </div>
          </div>
